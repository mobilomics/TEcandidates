...
do
	...
	if [ "$mode" == "SE" ];
        then
		readFileBasename=$(basename $readFile)
		samFile=${readFileBasename/.fastq/.sam}
		bowtie2Summary=${readFileBasename/.fastq/.bt2_summary}
		bamFile=${samFile/.sam/.bam}
		filteredBamFile=${samFile/.sam/_filtered.bam}
		filteredFastq=${filteredBamFile/.bam/.fastq}
		...
		then
			...
			#samtools view $samFile --threads $threads -b > $bamFile
			#bedtools intersect -a $bamFile -b $TEannotation -u > $filteredBamFile
		...
		filteredReadFiles=$filteredReadFiles""$filteredFastq" "
        	...
        	#Placeholder for Paired-end reads
		mate1=$readFileBasename
                mate2=${mate1/_1.fastq/_2.fastq}

		samFile=${readFileBasename/_1.fastq/.sam}		
		bowtie2Summary=${readFileBasename/_1.fastq/.bt2_summary}
		bamFile=${samFile/.sam/.bam}
		filteredBamFile=${samFile/.sam/_filtered.bam}
		filteredFastq=${filteredBamFile/.bam/.fastq}
		...
		if [ -e $filteredFastq ];
			...
			exit
			#bowtie2 -N 1 --threads $threads -x $BT2_basename -1 $mate1 -2 $mate2 2> $bowtie2Summary|samtools view 	
...
do
	...
	cmd="perl $TEcandidatesDIR/utils/parseFasta.pl $trinityOutput > $parsed_trinityOutput"
	echo -e "CMD: $cmd\n"
